...
class LaboratoryDashboardController extends Controller
{
    ...
    public function index(): View
    {
        ...
        $inProgressCount = Invoice::where('department', 'lab')
            ->where('status', Invoice::STATUS_IN_PROGRESS)->count();
        $completedToday = Invoice::where('department', 'lab')
            ->where('status', Invoice::STATUS_COMPLETED)
            ->whereDate('updated_at', today())->count();
        $paidToday = Invoice::where('department', 'lab')
            ->where('status', Invoice::STATUS_PAID)
            ->whereDate('updated_at', today())->count();

        $lowStockItems = InventoryItem::where('department', 'laboratory')
            ->where('minimum_stock_level', '>', 0)
            ->get()
            ->filter(fn ($item) => $item->stockMovements()->sum('quantity') < $item->minimum_stock_level)
            ->count();

        $pendingProcurements = ProcurementRequest::where('department', 'laboratory')
            ->where('status', 'pending')->count();

        // Paid invoices awaiting work (upfront-payment flow)
        $paidAwaitingWork = Invoice::where('department', 'lab')
            ->where('status', Invoice::STATUS_PAID)
            ->whereNull('performed_by_user_id')
            ->count();

        $workQueue = Invoice::where('department', 'lab')
            ->where(function ($q) {
        ...
        $recentCompleted = Invoice::where('department', 'lab')
            ->where('status', Invoice::STATUS_COMPLETED)
            ->with('patient')
            ->latest()
            ->limit(10)
