@extends('layouts.app')
@section('title', 'Radiology Dashboard — ' . config('app.name'))
...
<div class="container mt-4">
    ...
    <div class="page-header d-flex justify-content-between align-items-center flex-wrap gap-2">
        <div>
            <h2 class="mb-1"><i class="bi bi-radioactive me-2" style="color:var(--accent-warning);"></i>Radiology Dashboard</h2>
            <p class="page-subtitle mb-0">Manage imaging orders, reports & supplies <span class="auto-poll-indicator ms-2" role="status" aria-label="Auto-refreshing"><span class="auto-poll-dot"></span>Live</span></p>
        </div>
        <a href="{{ route('radiology.invoices.index') }}" class="btn btn-primary btn-sm"><i class="bi bi-list-check me-1"></i>All Scans</a>
    </div>
        ...
        <div class="col-6 col-md-2">
            <div class="card hover-lift" style="border-left:3px solid var(--accent-primary);">
                <div class="card-body glass-stat text-center py-2">
                    ...
                    <div class="stat-label" style="font-size:0.7rem;">Scans This Month</div>
                </div>
            </div>
        </div>
        @if(!empty($kpi['nps']))
    ...
    @php
        $tasks = collect();
        if (($pendingInvoices ?? 0) > 0) $tasks->push(['icon' => 'bi-hourglass-split', 'color' => 'warning', 'count' => $pendingInvoices, 'label' => 'Pending scans to start', 'url' => route('radiology.invoices.index')]);
        if (($paidAwaitingWork ?? 0) > 0) $tasks->push(['icon' => 'bi-cash-coin', 'color' => 'info', 'count' => $paidAwaitingWork, 'label' => 'Paid — awaiting work', 'url' => route('radiology.invoices.index')]);
        if (($inProgressCount ?? 0) > 0) $tasks->push(['icon' => 'bi-arrow-repeat', 'color' => 'primary', 'count' => $inProgressCount, 'label' => 'Scans in progress', 'url' => route('radiology.invoices.index')]);
        ...
        <div class="col-lg-8">
            <div class="card mb-4 fade-in delay-3">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <span><i class="bi bi-camera-reels me-2" style="color:var(--accent-warning);"></i>Imaging Work Queue</span>
                    <a href="{{ route('radiology.invoices.index') }}" class="btn btn-sm btn-outline-primary">View All Scans</a>
                </div>
                <div class="card-body p-0">
                    @if(($workQueue ?? collect())->isEmpty())
                        <div class="empty-state py-4">
                            ...
                            <p class="mb-0">No pending or in-progress scans</p>
                        </div>
                    @else
                                ...
                                <tbody>
                                    @foreach($workQueue as $invoice)
                                        <tr>
                                            ...
                                            <td>
                                                <a href="{{ route('radiology.invoices.show', $invoice) }}" class="btn btn-sm btn-info"><i class="bi bi-play-circle me-1"></i>Work</a>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            ...
                            <tbody>
                                @foreach($recentCompleted as $invoice)
                                    <tr>
                                        ...
                                        <td>{{ currency($invoice->total_amount) }}</td>
                                        <td>
                                            <a href="{{ route('radiology.invoices.show', $invoice) }}" class="btn btn-sm btn-outline-info">View</a>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
            ...
            <div class="card mb-4 fade-in delay-4">
                ...
                <div class="card-body">
                    <div class="quick-actions" style="grid-template-columns: 1fr;">
                        <a href="{{ route('radiology.invoices.index') }}" class="quick-action-btn">
                            <i class="bi bi-camera-reels" style="color:var(--accent-primary);"></i>View All Scans
                        </a>
                        <a href="{{ route('inventory.index') }}" class="quick-action-btn">
                            <i class="bi bi-box-seam" style="color:var(--accent-info);"></i>Scan Catalog
                        </a>
