@extends('layouts.app')
@section('title', 'Radiology Orders — ' . config('app.name'))

@section('content')
<div class="container mt-4">
    <div class="page-header d-flex justify-content-between align-items-center flex-wrap gap-2 fade-in">
        <div>
            <h2 class="mb-1"><i class="bi bi-broadcast me-2" style="color:var(--accent-secondary);"></i>Radiology Orders</h2>
            <p class="page-subtitle mb-0">Review and complete radiology imaging orders</p>
        </div>
    ...
    <div class="card fade-in delay-1">
        <div class="card-header d-flex justify-content-between align-items-center">
            <span><i class="bi bi-list-ul me-2" style="color:var(--accent-secondary);"></i>Imaging Orders</span>
                ...
                <table class="table table-hover mb-0">
                    <thead>
                        <tr>
                            ...
                            <th class="sortable-th">Imaging Type</th>
                    ...
                    <tbody>
                        @foreach($invoices as $invoice)
                            <tr>
                                ...
                                <td style="color:var(--text-secondary);">{{ $invoice->service_name ?? 'Imaging N/A' }}</td>
                                ...
                                <td>
                                    <a href="{{ route('radiology.invoices.show', $invoice) }}" class="btn btn-sm btn-outline-primary me-1"><i class="bi bi-eye me-1"></i>View</a>
                                    @if($invoice->status !== 'completed')
                                        <form action="{{ route('radiology.invoices.mark-complete', $invoice) }}" method="POST" class="d-inline">
                                            @csrf
                                            <button type="submit" class="btn btn-sm btn-success" onclick="return confirm('Mark this imaging complete?')"><i class="bi bi-check-circle me-1"></i>Complete</button>
                                        </form>
                                    @endif
                                </td>
                            </tr>
    ...
    <div class="card fade-in delay-1">
        <div class="card-body">
            <div class="empty-state py-5">
                ...
                <h5>No imaging orders</h5>
                <p class="mb-0">All imaging work is complete or none assigned yet.</p>
            </div>
        </div>
    </div>
    @endif
