@extends('layouts.app')
@section('title', 'Radiology Catalog — ' . config('app.name'))
...
<div class="container py-4">
    ...
    <div class="page-header mb-4">
        <div>
            <h1 class="page-title"><i class="bi bi-tags me-2"></i>Imaging Procedure Pricing</h1>
            <p class="text-muted mb-0">Manage radiology procedures &amp; pricing — changes apply to new invoices immediately</p>
        </div>
        <a href="{{ route('radiology.catalog.create') }}" class="btn btn-primary">
        ...
        @foreach($pendingChanges as $change)
            <div class="d-flex justify-content-between align-items-center mb-1 p-2 rounded" style="background:var(--glass-bg);">
                <div>
                    ...
                    <span class="fw-medium">{{ $change->change_payload['name'] ?? 'Procedure' }}</span>
        ...
        <div class="row g-3 mb-4">
            <div class="col-6 col-md-3">
                <div class="glass-stat">
                    ...
                    <div class="glass-stat-label">Total Procedures</div>
                </div>
            </div>
            <div class="col-6 col-md-3">
                <div class="glass-stat">
            ...
            <div class="glass-panel mb-4">
                <div class="catalog-category-header">
                    <div class="d-flex align-items-center gap-2">
                        ...
                        <span class="badge-glass ms-2">{{ $tests->count() }} {{ Str::plural('procedure', $tests->count()) }}</span>
                    </div>
                </div>
                        ...
                        <tbody>
                            @foreach($tests as $test)
                                <tr class="{{ !$test->is_active ? 'opacity-50' : '' }}">
                                    ...
                                    <td class="text-end">
                                        <a href="{{ route('radiology.catalog.edit', $test) }}" class="btn btn-sm btn-outline-primary" title="Edit pricing">
    ...
    @else
        <div class="glass-panel text-center py-5">
            <i class="bi bi-radioactive" style="font-size:3rem;opacity:0.3;"></i>
            <h5 class="mt-3">No Imaging Procedures Configured</h5>
            <p class="text-muted mb-3">Start by adding your first procedure to the catalog.</p>
            <a href="{{ route('radiology.catalog.create') }}" class="btn btn-primary">
                <i class="bi bi-plus-circle me-1"></i> Add Your First Procedure
            </a>
        </div>
    @endif
</div>
