@extends('layouts.app')
@section('title', 'Laboratory Equipment — ' . config('app.name'))

@section('content')
<div class="container mt-4">
    <div class="page-header d-flex justify-content-between align-items-center flex-wrap gap-2 fade-in">
        <div>
            <h2 class="mb-1"><i class="bi bi-tools me-2" style="color:var(--accent-info);"></i>Laboratory Equipment</h2>
            <p class="page-subtitle mb-0">Manage lab instruments & maintenance schedules</p>
        </div>
        <a href="{{ route('laboratory.equipment.create') }}" class="btn btn-primary btn-sm"><i class="bi bi-plus-circle me-1"></i>Add Equipment</a>
    </div>
                    ...
                    <tbody>
                        @foreach($equipment as $item)
                            <tr>
                                ...
                                <td>
                                    <a href="{{ route('laboratory.equipment.edit', $item) }}" class="btn btn-sm btn-outline-primary"><i class="bi bi-pencil me-1"></i>Edit</a>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
    ...
    @else
        <div class="glass-card text-center py-5 fade-in delay-1">
            <i class="bi bi-tools" style="font-size:2.5rem; color:var(--accent-info); opacity:0.5;"></i>
            <h5 class="mt-3">No Equipment Registered</h5>
            <p class="mb-3" style="color:var(--text-muted);">Start by adding your first piece of lab equipment.</p>
            <a href="{{ route('laboratory.equipment.create') }}" class="btn btn-primary btn-sm"><i class="bi bi-plus-circle me-1"></i>Add Equipment</a>
        </div>
    @endif
</div>
