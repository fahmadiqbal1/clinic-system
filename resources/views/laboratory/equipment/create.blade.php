@extends('layouts.app')
@section('title', 'Add Lab Equipment — ' . config('app.name'))

@section('content')
<div class="container mt-4">
    <div class="page-header d-flex justify-content-between align-items-center flex-wrap gap-2 fade-in">
        <div>
            <h2 class="mb-1"><i class="bi bi-plus-circle me-2" style="color:var(--accent-success);"></i>Add Laboratory Equipment</h2>
            <p class="page-subtitle mb-0">Register new lab instrument</p>
        </div>
        <a href="{{ route('laboratory.equipment.index') }}" class="btn btn-outline-secondary btn-sm"><i class="bi bi-arrow-left me-1"></i>Back</a>
    </div>

    <div class="glass-card p-4 fade-in delay-1" style="max-width:900px;">
            <form method="POST" action="{{ route('laboratory.equipment.store') }}">
                @csrf
                <div class="row g-3">
                    <div class="col-md-6">
                        <label class="form-label">Equipment Name <span class="text-danger">*</span></label>
                        <input type="text" name="name" class="form-control @error('name') is-invalid @enderror"
                            value="{{ old('name') }}" required placeholder="e.g. Hematology Analyzer">
                        @error('name')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>
                    <div class="col-md-3">
                        <label class="form-label">Model</label>
                        <input type="text" name="model" class="form-control @error('model') is-invalid @enderror"
                            value="{{ old('model') }}" placeholder="e.g. Sysmex XN-1000">
                    ...
                    <div class="col-12">
                        ...
                        <textarea name="notes" class="form-control @error('notes') is-invalid @enderror"
                            rows="3" placeholder="Maintenance schedule, calibration notes, etc.">{{ old('notes') }}</textarea>
                ...
                <div class="mt-4 d-flex gap-2">
                    ...
                    @endif
                    <a href="{{ route('laboratory.equipment.index') }}" class="btn btn-outline-secondary">Cancel</a>
                </div>
            </form>
    </div>
</div>
