...
<div class="container mt-4">
    <div class="page-header mb-4 fade-in">
        <h1><i class="bi bi-pencil me-2"></i>Edit Service</h1>
        <p class="page-subtitle">Update {{ $service->name }}</p>
    </div>

    <div class="glass-card p-4 fade-in delay-1" style="max-width: 700px;">
        <form method="POST" action="{{ route('owner.service-catalog.update', $service) }}">
            @csrf
            @method('PATCH')
            ...
            <div class="mb-3">
                ...
                <select name="department" id="department" class="form-select @error('department') is-invalid @enderror" required>
                    ...
                    <option value="consultation" {{ old('department', $service->department) === 'consultation' ? 'selected' : '' }}>Consultation</option>
                    <option value="lab" {{ old('department', $service->department) === 'lab' ? 'selected' : '' }}>Laboratory</option>
                    <option value="radiology" {{ old('department', $service->department) === 'radiology' ? 'selected' : '' }}>Radiology</option>
                    <option value="pharmacy" {{ old('department', $service->department) === 'pharmacy' ? 'selected' : '' }}>Pharmacy</option>
                </select>
            ...
            <div class="row">
                ...
                <div class="col-md-4 mb-3">
                    ...
                    <input type="text" name="hs_code" id="hs_code" class="form-control @error('hs_code') is-invalid @enderror"
                           value="{{ old('hs_code', $service->hs_code) }}" placeholder="e.g. 9018.90.10">
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <input type="number" name="price" id="price" class="form-control @error('price') is-invalid @enderror" value="{{ old('price', $service->price) }}" step="0.01" min="0" required>
                    @error('price') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                <div class="col-md-6 mb-3">
                    <label for="turnaround_time" class="form-label">Turnaround Time</label>
                    <input type="text" name="turnaround_time" id="turnaround_time" class="form-control @error('turnaround_time') is-invalid @enderror" value="{{ old('turnaround_time', $service->turnaround_time) }}" placeholder="e.g. 2 hours, 1 day">
